...
1. 写 `/etc/udev/rules.d/99-canable.rules`（slcan `16d0:117e` / candleLight `1d50:606f` / DFU `0483:df11` 三种 VID:PID 的 USB 权限）
2. 写 `/etc/udev/rules.d/80-candleLight-up.rules`（CANable 插上按 `ATTRS{idVendor}=="1d50"` 匹配自动 `ip link set %k type can bitrate 1000000 up`，不管内核把它分到 `can0`/`can1`/… 都对）
3. 清理旧版 `can0-up.service`（硬编 `can0` 名字，在 Jetson 上会误拉 SoC 内置 MTTCAN，必须删）
...
# Jetson: 还要先装 gs_usb 驱动 (见下), 否则 CANable 连 netdev 都出不来
cd gs_usb_mod && sudo bash build.sh install && cd ..

# 插上/拔插一次 CANable, 验证
uv run detect.py
```

### Jetson 专属：`gs_usb` 驱动必须自己编

JetPack 6 (L4T 36.x) 的 NVIDIA 内核**故意删掉了 `gs_usb` 驱动**（同目录下 `kvaser_usb` / `peak_usb` 都留着，就这个没有）。后果：CANable 插上 `lsusb` 看得见，但**不会**生成 `canN` netdev —— 没驱动 claim 它。

另外 Jetson Orin 的 SoC 内置了 MTTCAN 控制器，`can0` 默认归它，CANable 会被分到 `can1`（或更高）。**本仓库所有脚本按驱动名自动识别 CANable 接口**（不再硬编 `can0`），详见 `device.py::find_can_interface()`。

修复：`damiao/gs_usb_mod/` 是一个 OOT 模块子项目，从 mainline 拉对应版本的 `gs_usb.c` 编成 `.ko`，装到 `/lib/modules/$(uname -r)/extra/`。详见 `gs_usb_mod/README.md`。

工作机（Ubuntu 24 泛型内核）无此坑 —— Ubuntu 的 `linux-modules-*` 默认带 `gs_usb`，CANable 插上直接是 `can0`。
...
```bash
uv run detect.py                      # 完整检测（USB + 所有 CAN 接口 + 电机探活, 用出厂默认 0x01/0x00）
uv run detect.py --skip-motor         # 只做 USB + CAN 接口列表
uv run detect.py --id 5               # 探活 FR_HFE (motor=0x05, master=0x15)
uv run detect.py --motor-id 0x05      # 非 +0x10 约定的电机, 配合 --master-id 用
uv run detect.py -i can1              # 强制用某个接口 (默认按驱动自动挑 gs_usb)
```

枚举 CANable 2.0（两种 VID:PID）、**列出所有 CAN netdev 并标注驱动**（`gs_usb`=CANable / `mttcan`=Jetson SoC 内置 / `slcan`=串口仿真）、按驱动优先级自动挑 CANable 接口、发送 1-shot 探活帧并读取电机反馈。

> 临时覆盖自动识别：`DAMIAO_CAN_IF=can1 uv run detect.py` 或脚本加 `-i can1`。
...
| CAN 接口列表只有 `can0 driver=mttcan` (Jetson) | 缺 `gs_usb` 驱动。跑 `cd gs_usb_mod && sudo bash build.sh install`，拔插 CANable 再查 |
| `can1` (CANable) 一直 DOWN | `setup.sh` 的 udev 自动拉 1M 规则没装或没触发。先 `sudo bash setup.sh`，再拔插一次 |
| `detect.py` 把电机探活发到了错的接口 | 用 `-i can1` 显式指定，或 `DAMIAO_CAN_IF=can1` 环境变量覆盖 |
| 固件仍是 slcan (`16d0:117e`) | 没 CAN netdev 生出来。先 `fw_update.py` 烧 candleLight |
